...
**Search summary:**\n  
Search file "`{r} r$File[1]`" (`{r} r$Size[1]`GB) is last modified `{r} r$Date[1]`.  
Search contains `{r} r$Files[1]` files across `{r} r$Conditions[1]` conditions.  
Files are searched against database(s) `{r} paste0(r %>% separate_rows(FASTA, sep=";") %>% distinct(FASTA) %>% pull(FASTA), collapse="; ")` (`{r} paste0(r %>% separate_rows(Species, sep=";") %>% distinct(Species) %>% pull(Species), collapse="; ")`) with `{r} r$Errors[1]` errors and `{r} r$Warnings[1]` warnings.
...
This is based on a grouping per protein in each group. So a 1 means a protein identified with only one peptide across all samples in this group.
