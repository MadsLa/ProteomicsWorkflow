...
Packages are collected in two files
```{r}
source("source.R")
source("functions.R")
```
...
```{r,data_load}
file = "raw/20250826_081536_20220329_QE5_YABH_PlasmaProt_Groningen_pEF-Healthy_SP_MQ+hybrid search_Report.tsv"
dfBGS <- MDBL_ReadBGSReport(file)
```

## Simple summaries
```{r}
MDBL_PrintSummary(dfBGS)
```
...
Table is only printed if number of samples is 20 or less.
```{r, table_samples}
dfBGS %>% 
...
if (nrow(t)<=20) {
  ...
  print("Printing first 20 samples in set")
  head(t)
}
```
...
### Peptides identified per second across gradient

The following plot will show how optimized method is for samples and consistancy between samples. By default the counts are averaged across 10s (6 data points per minute)
```{r}
average_across_s = 10
my_bins <- c(seq(0, max(dfBGS$EG.ApexRT)+0.1, by=0.0166667*average_across_s))

dfBGS %>%  
  mutate(binned = cut(EG.ApexRT, breaks = my_bins)) %>%
  mutate(bin = str_remove(binned, ",.*")) %>% 
  mutate(bin = as.numeric(str_remove(bin, "[(]" ))) %>% 
  count(R.Condition, R.FileName, bin) %>% 
  mutate(n=n/average_across_s)-> t
  
t %>% ggplot(aes(bin, n, group=R.FileName, color=R.FileName))+
  geom_line()+
  #scale_x_continuous(breaks = seq(0,99,1))+
  facet_wrap(~R.Condition)+
  labs(title="Peptides identified per second", subtitle=paste0("Averaged across ", average_across_s, " second(s)"), x="Gradient time", y="Peptides identified")+
  theme_minimal()+
...
```{r, proteins_per_condition_barplot}
dfBGS %>% 
...
Density plot showing the distribution of all proteins identified per sample.
...
### S-curve
Intensity of all proteins identified per sample arranged after intensity.
```{r}
dfBGS %>% 
...
### Order of magnitudes covered per condition
```{r}
dfBGS %>% 
  distinct(R.Condition, PG.ProteinAccessions, R.FileName, PG.Quantity) %>% 
  filter(!is.na(PG.Quantity)) %>% 
  group_by(PG.ProteinAccessions, R.Condition) %>%
  summarise(median = median(PG.Quantity)) %>% 
  group_by(R.Condition) %>% 
  summarise(Decades = log10(max(median) - min(median)))
```

### Peptides per protein

This is based on a grouping per protein in each group. So a 1 means a protein identified with only 1 peptide across all samples in this group.
```{r}
dfBGS %>% 
...
dfBGS %>% 
  ...
  filteringVV_AllSamples(filter_value = 1) -> t # filter to 100% VV

dfBGS %>% 
  distinct(R.FileName, PG.Quantity, PG.ProteinAccessions) %>% 
  filter(PG.ProteinAccessions %in% as.list(t$PG.ProteinAccessions)) %>% 
  pivot_wider(names_from = R.FileName, values_from = PG.Quantity)  -> umap.prep
